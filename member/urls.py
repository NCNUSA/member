...
urlpatterns = [
    ...
    path('Group/', include([
        ...
        path('update/', include([
            path('<str:gp>/<str:sid>/', views.group_member_update,
                 name='gp.update.member'),
            path('<str:gp>/', views.group_member_list_update,
                 name='gp.update.list'),
        ])),
    ])),

    # 連結到 Google 表單爬蟲後做檢查
    path('GoogleSheet/', include([
        ...
        path('update/<int:uid>', views.google_sheet_edit,
             name='GoogleSheet.update'),
    ])),
