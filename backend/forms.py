...
class ColPositionField(forms.IntegerField):
    def to_python(self, value):
        ...
        if value.isdigit():
            ...
            raise forms.ValidationError(
                _("Invalid value,%(value)s 請輸入 0~50 間整數"), code="invalid", params={"value": value}
            )
    ...
    def validate(self, value):
        ...
        if value < 0 or value > 50:
            raise forms.ValidationError(
                _("Invalid value: %(value)s. 請輸入 0~50"), code="invalid", params={"value": value}
            )
...
class SheetCheckForm(forms.Form):
    ...
    def clean_spreadsheet(self):
        ...
        if data.size > 1024 * 1024:
            raise forms.ValidationError(_("檔案過大，請使用小於 1 MB 的檔案"))
        # file type should be xlsx or xls
        if not (
            data.content_type == "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
            or data.content_type == "application/vnd.ms-excel"
        ):
            raise forms.ValidationError(_("請上傳 xls/xlsx 格式之檔案"))
        return data
